...
function PasswordField({ id, label, value, onChange, error, autoComplete, hint }) {
    ...
    return (
        <div>
            ...
            {error && <p className="mt-1.5 text-xs text-red-500 font-medium flex items-center gap-1"><span>⚠</span>{error}</p>}
            {hint && !error && <p className="mt-1.5 text-[10px] text-slate-400">{hint}</p>}
        </div>
    );
}
...
export default function Register() {
    ...
    return (
        <GuestLayout>
            <Head title="Inscription · UniConnect" />

            {/* Header */}
            <div className="text-center mb-6">
                <h1 className="text-lg font-black text-slate-900">Rejoindre UniConnect 🎓</h1>
                <p className="text-sm text-slate-500 mt-1 leading-relaxed">
                    Créez votre espace avec votre email institutionnel SupMTI
                </p>
            </div>

            {/* Étapes visuelles */}
            <div className="flex items-center justify-center gap-2 mb-6">
                {['Identité', 'Sécurité', 'Accès'].map((step, i) => (
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div className="flex items-center gap-3 py-1">
                    ...
                    <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Sécurité</span>
                    <div className="flex-1 h-px bg-gray-100" />
                </div>
                ...
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3.5">
                    <div className="flex items-start gap-2.5">
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-emerald-800 mb-0.5 uppercase tracking-wide">Engagement éthique RGPD</p>
                            <p className="text-[10px] text-emerald-700 leading-relaxed">
                                Vos données (nom, email) sont <strong>minimisées</strong>, jamais revendues, et vous bénéficiez du <strong>droit à l'oubli</strong> à tout moment. Zéro publicité, zéro tracking.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2.5">
                    ...
                    <p className="text-[10px] text-blue-700 font-semibold">
                        Accès vérifié @supmti.ma uniquement · Session chiffrée HTTPS
                    </p>
                </div>
